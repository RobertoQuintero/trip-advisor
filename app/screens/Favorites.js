...
const Favorites = (props) => {
  ...
  //
  const [reloadData, setReloadData] = useState(false); //interruptor
  ...
  if (!userLoged) {
    return <UserNoLogged navigation={props.navigation} />;
  }
  if (!restaurants) {
    ...
    <View style={styles.viewBody}>
      {restaurants ? (
        ...
        <View style={styles.loaderRestaurants}>
          ...
          {/* funcionaria correctamente si no existiera el loading */}
        </View>
      )}
      <Toast ref={toastRef} position="center" opacity={0.9} />
      <Loading text="Eliminando restaurant" isVisible={isLoading} />
    </View>
  );
};
